  ...
  useEffect(() => {
    const fetchDouble = async () => {
      try {
        ...
        // البحث عن الـ ID الخاص بقسم النفرين
        const doubleCategory = categoriesList.find((c: any) => 
          (c.name || c.title || '').includes('نفرين') || (c.name || c.title || '').includes('مزدوج')
        );
        ...
        if (doubleCategory) {
          const filtered = allProducts.filter((p: any) => {
            ...
            return Number(pCatId) === Number(doubleCategory.id);
          });
