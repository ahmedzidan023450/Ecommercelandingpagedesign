    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            using (var scope = app.Services.CreateScope())
            {
                ...
                var requiredCategories = new[] { "غرف مفردة", "غرف مزدوجة", "غرف أطفال", "كنب ومجالس", "غرف سحاب" };
                foreach (var cat in requiredCategories)
                {
                    if (!context.Categories.Any(c => c.Name == cat))
                    {
                        context.Categories.Add(new Furniture_E_Commerce.Models.Category { Name = cat, Description = cat });
                    }
                }
                context.SaveChanges();
            }
