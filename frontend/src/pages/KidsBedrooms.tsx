...
export default function KidsBedrooms() {
  // 1. جلب الأقسام (مكيّشة من الصفحات اللي قبلها لو العميل كان بيتصفح)
  ...
  // 3. دمج وفلترة البيانات بذكاء باستخدام useMemo
  const kidsProducts = useMemo(() => {
    if (!categoriesData || !productsData) return [];

    // البحث عن قسم الأطفال
    const kidsCategories = categoriesData.filter((c: any) =>
      (c.name || c.title || '').includes('أطفال') || (c.name || c.title || '').includes('اطفال')
    );

    if (kidsCategories.length === 0) return [];
    
    const categoryIds = kidsCategories.map((c: any) => Number(c.id));
  ...
  }, [categoriesData, productsData]); // الفلترة مش هتشتغل تاني إلا لو البيانات دي اتغيرت

  // تجميع حالات التحميل والخطأ
    ...
    <div className="min-h-screen bg-gray-50 py-12" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-blue-950 mb-4">غرف نوم أطفال</h1>
          <p className="text-gray-600 text-lg">غرف أطفال وشباب مبهجة وعملية لجميع الأعمار.</p>
        </div>
