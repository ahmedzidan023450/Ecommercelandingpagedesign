...
export default function WardrobeRooms() {
  ...
  const singleProducts = useMemo(() => {
    ...
    // البحث عن قسم غرف سحاب
    const wardrobeCategories = categoriesData.filter((c: any) =>
      (c.name || c.title || '').includes('سحاب') || (c.name || c.title || '').includes('دولاب')
    );
