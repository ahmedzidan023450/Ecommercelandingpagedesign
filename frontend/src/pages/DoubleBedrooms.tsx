...
export default function DoubleBedrooms() {
  // 1. جلب الأقسام
  ...
  // 3. دمج وفلترة البيانات بذكاء (الـ useMemo بيمنع إعادة الحسابات بدون داعي)
  const doubleProducts = useMemo(() => {
    if (!categoriesData || !productsData) return [];

    // البحث عن قسم نفرين
    const doubleCategories = categoriesData.filter((c: any) =>
      (c.name || c.title || '').includes('مزدوج') || (c.name || c.title || '').includes('نفرين')
    );

    if (doubleCategories.length === 0) return [];
    
    const categoryIds = doubleCategories.map((c: any) => Number(c.id));
  ...
  }, [categoriesData, productsData]); // الفلترة هتتحدث بس لو الداتا اتغيرت
    ...
    <div className="min-h-screen bg-gray-50 py-12" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-blue-950 mb-4">غرف نوم نفرين</h1>
          <p className="text-gray-600 text-lg">أفخم غرف النوم المزدوجة والمصممة لراحتكم.</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {doubleProducts.map((product: any) => (
